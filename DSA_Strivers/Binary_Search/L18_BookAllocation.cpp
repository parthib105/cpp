...
bool canAllocate(int maxPages, int n, int m, vector<int>& arr)
{
...
int findPages(vector<int>& arr, int n, int m) 
{
    ...
    int maxElm = INT_MIN, sumArr = 0;
    for (auto elm : arr)
    {
        sumArr += elm;
        maxElm = max(maxElm, elm);
    }

    int low = maxElm, high = sumArr, ans = -1;
    while (low <= high)
    {
        int mid = low + (high - low) / 2;
        if (canAllocate(mid, n, m, arr))
        {
            ans = mid;
            high = mid - 1;
        }
