/*
    ...
    You are given an array/list of non-negative integers 'ARR' representing the amount of money of each house. Your task is
    to return the maximum amount of money Mr. X can rob tonight without alerting the police.
    ...
    (ii) Given the input array arr[] = {1, 2, 3, 1} the output will be 4 because Mr X rob house 1 (money = 1) and then rob house
    3 (money = 3).
...
long long int solveSO(vector<int> &arr)
{
    ...
    {
        long long int curr = max(prev2 + arr[i], prev1);
        prev2 = prev1;
        prev1 = curr;
    }
    return prev1;
...
long long int houseRobber(vector<int> &valueInHouse)
{
    ...
    return max(solveSO(fst), solveSO(sec));
}

int main()
{
    vector<int> valueInHouse = {1, 5, 1, 2, 6};
    cout << "Robbed Money : " << houseRobber(valueInHouse) << endl;
    return 0;
}